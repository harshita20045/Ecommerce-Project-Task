...
export const verifyGmail = async (request, response, next) => {
  try {
    ...
    return response
      .status(200)
      .json({ message: "Gmail Account Verified Successfully" });
  } catch (err) {
    return response.status(500).json({ error: "Internal Server Error" });
  }
};
...
export const authenticateUser = async (request, response, next) => {
  try {
    ...
    if (!user.isVerify)
      return response
        .status(401)
        .json({ error: "Unauthorized user | Account is not verified" });

    if (!user)
      return response
        .status(401)
        .json({ error: "Unauthorized user | Email id not found" });
